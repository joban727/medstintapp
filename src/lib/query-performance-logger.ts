...
// Placeholder for build time - never actually connects
const PLACEHOLDER_CONNECTION = "postgresql://placeholder:placeholder@localhost:5432/placeholder"

// Lazy database connection to prevent build-time errors
let _pool: Pool | null = null
let _db: ReturnType<typeof drizzle<typeof schema>> | null = null

function getDb() {
  if (!_db) {
    const connectionString = process.env.DATABASE_URL || PLACEHOLDER_CONNECTION
...
// Use proxy to make db access lazy
