...
export function createBusinessLogicError(message: string, code: string, context?: Record<string, any>): ClockError {
  return new ClockError(
...
export function createSystemError(message: string, retryable = true, context?: Record<string, any>): ClockError {
  return new ClockError(
    ClockErrorType.SYSTEM_ERROR,
    'SYSTEM_FAILURE',
    message,
    retryable,
    context
...
export function createDatabaseError(message: string, operation?: string, retryable = true): ClockError {
  return new ClockError(
...
export function createSecurityError(message: string, eventType: SecurityEventType, context?: Record<string, any>): SecurityError {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const isRetryable = shouldRetry 
          ? shouldRetry(error)
          : error instanceof ClockError ? error.retryable : true
