...
export class NeonRateLimiter {
   ...
   */
  async checkLimitFromRequest(request: NextRequest, userId?: string): Promise<RateLimitResult> {
...
export async function cleanupExpiredRateLimits(): Promise<number> {
  try {
    const result = await db.delete(rateLimits).where(sql`${rateLimits.windowEnd} < NOW()`)
