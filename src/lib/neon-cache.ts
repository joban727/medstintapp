...
class NeonCacheManager {
    ...
    async markSessionActive(sessionId: string, userId?: string, expiresAtMs?: number): Promise<void> {
        ...
        try {
            ...
            await this.set(key, { sessionId, userId, expiresAt: expirationMs, lastSeen: now }, Math.ceil((expirationMs - now) / 1000))
        } catch (error) {
    ...
    async getActiveSessionCount(): Promise<number> {
        try {
            const result = await db
                ...
                .from(cacheEntries)
                .where(sql`${cacheEntries.key} LIKE ${`${CACHE_PREFIXES.SESSION}:active:%`} AND ${cacheEntries.expiresAt} > NOW()`)
