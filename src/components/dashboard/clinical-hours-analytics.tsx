...
export function ClinicalHoursAnalytics({
    ...
    // Generate weekly data from actual timeRecords
    const generateWeeklyData = (): WeeklyHoursData[] => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            // Sum hours for this day from actual timeRecords
            const dayHours = timeRecords
    ...
    const generateDailyData = (): DailyHoursData[] => {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            const dayActivities = dayRecords.reduce((sum, r) => sum + (r.activities?.length || 0), 0)
            const hasPending = dayRecords.some(r => r.status === 'PENDING')

            data.push({
                date: dateStr,
                hours: dayHours,
                activities: dayActivities || (dayHours > 0 ? 1 : 0),
    ...
    const generateRotationData = (): RotationHoursData[] => {
        // Group hours by rotation from actual timeRecords
        const rotationMap = new Map<string, { hours: number; specialty: string }>()

        for (const record of timeRecords) {
            if (record.rotation?.name) {
                const current = rotationMap.get(record.rotation.name) || { hours: 0, specialty: record.rotation.specialty || '' }
                current.hours += record.totalHours || 0
                rotationMap.set(record.rotation.name, current)
            }
        }
        ...
        return Array.from(rotationMap.entries()).map(([name, data]) => {
            const target = 80 // Default target hours per rotation
            return {
                rotation: name,
                hours: data.hours,
                target,
                percentage: Math.round((data.hours / target) * 100),
    ...
    const generateActivityData = (): ActivityBreakdownData[] => {
        // Group hours by activity type from actual timeRecords
        const activityMap = new Map<string, number>()
        let total = 0

        for (const record of timeRecords) {
            if (record.activities && record.activities.length > 0) {
                const hoursPerActivity = (record.totalHours || 0) / record.activities.length
                for (const activity of record.activities) {
                    activityMap.set(activity, (activityMap.get(activity) || 0) + hoursPerActivity)
                    total += hoursPerActivity
                }
            } else if (record.totalHours) {
                // Default to "Clinical Work" if no activities specified
                activityMap.set('Clinical Work', (activityMap.get('Clinical Work') || 0) + record.totalHours)
        ...
        return Array.from(activityMap.entries()).map(([activity, hours]) => ({
            activity,
    ...
    }, [timeRecords, totalHours, weeklyTarget])
