...
export const LocationStatusIndicator: React.FC<LocationStatusIndicatorProps> = ({
  ...
  const getStatusConfig = () => {
    ...
    if (isTracking) {
      return {
        icon: Navigation,
        color: accuracyLevel === 'high' ? 'text-success' : accuracyLevel === 'medium' ? 'text-warning' : 'text-error',
        bgColor: accuracyLevel === 'high' ? 'bg-success/10' : accuracyLevel === 'medium' ? 'bg-warning/10' : 'bg-error/10',
        label: 'Live Tracking',
        description: accuracyLevel === 'high' ? 'High accuracy' : accuracyLevel === 'medium' ? 'Medium accuracy' : 'Low accuracy'
      }
    }
      ...
      <div className={cn("relative p-2 rounded-full", status.bgColor)}>
        ...
        {isTracking && (
          <div className={cn(
            ...
            accuracyLevel === 'high' ? 'bg-success' : accuracyLevel === 'medium' ? 'bg-warning' : 'bg-error'
          )} />
        )}
      </div>
