...
export function TimeTracker({ currentRotation, todayTimeRecord, onTimeRecordUpdate }: TimeTrackerProps) {
  ...
  useEffect(() => {
    ...
    if (recordToUse) {
      try {
        const activities = JSON.parse(recordToUse.activities || "[]")
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data) {
          setOptimisticRecord(result.data)
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between rounded-lg bg-gray-50 p-4">
          <div>
            <p className="font-medium text-gray-600 text-sm">Current Time</p>
            ...
            <div className="flex items-center gap-2">
              {isClocked && (
                <>
                  ...
                  <p className="font-bold text-2xl text-green-600">{elapsedTime}</p>
                </>
              )}
              {isCompleted && (
                <>
              ...
              {!displayRecord && (
                <>
                  ...
                  <p className="font-medium text-gray-400 text-lg">Not Started</p>
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex gap-3">
          ...
          {isCompleted && (
            <div className="flex-1 py-2 text-center">
              <p className="text-gray-600 text-sm">Time tracking completed for today</p>
            </div>
          )}
        </div>
      </CardContent>
