...
export default async function SupervisorStudentsPage() {
  ...
  const studentDetailsPromises = supervisedStudents.map(async (student) => {
    if (!student.id) return { ...student, year: 1, gpa: "0.00", attendanceRate: 0 }

    try {
      const { timeRecords, evaluations, competencyAssignments } = await import(
        "@/database/schema"
      )
      ...
      return {
        ...student,
        year: 3, // Default to 3rd year medical student
        school: "Medical School", // Default school name
        gpa: "3.50", // Default GPA - would need separate academic records table
        attendanceRate,
        ...
        site: "Clinical Site", // TODO: Get site from rotation data
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground text-xs">
              <span className="text-green-600">{activeRotations}</span> active rotations
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="font-medium text-sm">High Risk</CardTitle>
            <AlertTriangle className="h-4 w-4 text-red-500" />
          </CardHeader>
          <CardContent>
            <div className="font-bold text-2xl text-red-600">{highRiskStudents}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="font-bold text-2xl">{avgPerformance}%</div>
            <p className="text-muted-foreground text-xs">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="font-medium text-sm">Sites Covered</CardTitle>
            <MapPin className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="font-bold text-2xl">
              {new Set(studentDetails.map((s) => s.site)).size}
            </div>
            <p className="text-muted-foreground text-xs">Clinical locations</p>
          </CardContent>
        </Card>
      </div>
