...
export async function GET(_request: NextRequest) {
  ...
  try {
    ...
    const completionRates = completionRatesResult.rows.map((row: CompletionRateRow) => ({
      step: row.step,
    ...
    const analyticsSummary = analyticsSummaryResult.rows.map((row: AnalyticsSummaryRow) => ({
      step: row.step,
      event_type: row.event_type,
      event_count: Number.parseInt(row.event_count) || 0,
      avg_duration_ms: Number.parseFloat(row.avg_duration_ms) || 0,
      min_duration_ms: Number.parseFloat(row.min_duration_ms) || 0,
      max_duration_ms: Number.parseFloat(row.max_duration_ms) || 0,
    ...
    const averageCompletionTime =
      Number.parseFloat(String(avgCompletionTimeResult.rows[0]?.avg_completion_time_ms || "0")) || 0
