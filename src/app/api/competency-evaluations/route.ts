...
import { competencyAssignments, evaluations, rotations, users } from "../../../database/schema"
import {
...
export const POST = withCSRF(async (request: NextRequest) => {
  try {
    ...
    if (userId !== validatedData.evaluatorId) {
      ...
      if (!isSuperAdmin && !isSchoolAdmin) {
        return NextResponse.json(
          { error: "Forbidden: Cannot submit as another user" },
          { status: 403 }
        )
      }
    }
    ...
    if (!validateSubmissionTarget(userId, assignment.userId, user.role || "")) {
      return NextResponse.json({ error: "Invalid submission target" }, { status: 403 })
    }
    ...
    await updateAssignmentProgress(assignment.userId, assignment.competencyId, progressStatus)
