...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const hasAccess =
      ...
      (user.role === "SCHOOL_ADMIN" && user.schoolId === timeRecord.studentSchoolId) ||
      (user.role === "CLINICAL_SUPERVISOR" && user.schoolId === timeRecord.studentSchoolId) ||
      user.role === "CLINICAL_PRECEPTOR" || // Preceptors can view records they supervise
      (user.role === "STUDENT" && user.id === timeRecord.studentId)
