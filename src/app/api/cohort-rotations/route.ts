...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  if (status) {
    conditions.push(
      eq(
        cohortRotationAssignments.status,
...
export const PUT = withErrorHandling(async (request: NextRequest) => {
  ...
  if (validatedData.clinicalSiteId !== undefined)
    updateData.clinicalSiteId = validatedData.clinicalSiteId
  if (validatedData.startDate !== undefined)
    updateData.startDate = new Date(validatedData.startDate)
  if (validatedData.endDate !== undefined) updateData.endDate = new Date(validatedData.endDate)
  if (validatedData.requiredHours !== undefined)
    updateData.requiredHours = validatedData.requiredHours
