...
export const POST = withErrorHandling(async (request: NextRequest) => {
    ...
    const clinicalSiteId = validatedData.clinicalSiteId || assignment.clinicalSiteId || template.defaultClinicalSiteId

    if (!clinicalSiteId) {
        return createErrorResponse("No clinical site specified for this rotation", HTTP_STATUS.BAD_REQUEST)
    }
    ...
    const studentsToAssign = studentsInCohort.filter(
    ...
    if (assignment.maxStudents && existingRotations.length + studentsToAssign.length > assignment.maxStudents) {
