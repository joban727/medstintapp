...
export async function GET(request: NextRequest) {
  try {
    ...
    const locations = await db
      ...
      .orderBy(
        clinicalSiteLocations.isPrimary,
        clinicalSites.name,
        clinicalSiteLocations.name
      )
    ...
    if (groupBySite) {
      const grouped = formattedLocations.reduce((acc, location) => {
        ...
        return acc
      }, {} as Record<string, { id: string; name: string; locations: any[] }>)
