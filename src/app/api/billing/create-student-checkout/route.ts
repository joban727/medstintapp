...
import { studentPlan } from "../../../../lib/payments/plans"
...
// POST /api/billing/create-student-checkout - Create Stripe Checkout session for student subscription
export async function POST(request: NextRequest) {
    try {
        ...
        if (user.role !== "STUDENT") {
            return NextResponse.json({ error: "Only students can subscribe to the student plan" }, { status: 403 })
        }

        // Check if user already has an active subscription
        if (user.subscriptionStatus && ["ACTIVE", "TRIAL", "GRANDFATHERED"].includes(user.subscriptionStatus)) {
