...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[API] DATABASE_URL preview:", `${process.env.DATABASE_URL?.substring(0, 50)}...`)
    ...
    console.error("[API] Unexpected error in user update:", error)
    console.error("[API] Error type:", typeof error)
    console.error("[API] Error constructor:", (error as DatabaseError)?.constructor?.name)
    console.error("[API] Error message:", error instanceof Error ? error.message : String(error))
    console.error("[API] Error code:", (error as DatabaseError)?.code)
    console.error("[API] Error constraint:", (error as DatabaseError)?.constraint)
    console.error("[API] Error detail:", (error as DatabaseError)?.detail)
    console.error("[API] Error stack:", error instanceof Error ? error.stack : "No stack trace")
    console.error("[API] Error cause:", error instanceof Error ? error.cause : "No cause")
    console.error("[API] ========================================================================")

    // Handle unexpected errors that occurred outside of transactions
    let errorMessage = "Unexpected error occurred"
    let statusCode = 500

    const msg = error instanceof Error ? error.message : String(error)

    if (typeof msg === "string" && (msg.includes("auth") || msg.includes("unauthorized"))) {
      errorMessage = "Authentication error"
      statusCode = 401
    } else if (typeof msg === "string" && msg.includes("JSON")) {
      errorMessage = "Invalid request format"
      statusCode = 400
    } else if (typeof msg === "string" && msg.includes("Clerk")) {
      errorMessage = "External authentication service error"
      statusCode = 502
    }
